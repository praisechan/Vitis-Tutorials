# 1 "/home/juchanlee/Vitis-Tutorials/Hardware_Acceleration/Feature_Tutorials/05-using-multiple-cu/reference-files/src/kernel/axi2stream.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 375 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/home/juchanlee/Vitis-Tutorials/Hardware_Acceleration/Feature_Tutorials/05-using-multiple-cu/reference-files/src/kernel/axi2stream.cpp" 2
...
void PixelStream2AXIBursts(
  STREAM_PIXELS& stream,
        ...
        U16 StrideInBytes,
        AXIMM aximm);
# 7 "/home/juchanlee/Vitis-Tutorials/Hardware_Acceleration/Feature_Tutorials/05-using-multiple-cu/reference-files/src/kernel/axi2stream.cpp" 2
  ...
  STREAM_PIXELS& stream)
{
 ...
 const int AXIMM_DATA_BUFF_SZ = (1920+(256/8)-1)/(256/8);
 const int PIXELS_PER_AXIMM = (256/8);

 ap_uint<256> buff[AXIMM_DATA_BUFF_SZ];

    int yoffset = 0;
 int loopWidth = (WidthInBytes+(256/8)-1)/(256/8);
 int widthInPix = WidthInBytes;
 int remainPix = widthInPix%(4*(256/32));
 remainPix = (remainPix==0) ? (4*(256/32)) : remainPix;
        ...
        AXIMM aximm)
{
 ...
 int loopWidth = (WidthInBytes+(256/8)-1)/(256/8);
 int widthInPix = WidthInBytes;
 int remainPix = widthInPix%(256/8);
 remainPix = (remainPix==0) ? (256/(8*1)) : (remainPix/1);
...
#pragma HLS loop_flatten off
 ...
 pixels2bytes: for (int x = 0; x < loopWidth; x++)
  {
#pragma HLS pipeline II=PIXELS_PER_AXIMM
 VITIS_LOOP_94_1: for (int i=0; i<(256/8); i++)
   {
